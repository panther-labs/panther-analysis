...
def time_shift_json_logs(event_time_shift, logs, log_type):
    ...
    for log in logs:
        log_event_time = datetime.strptime(log[event_time_attr], event_time_format).replace(
            tzinfo=timezone.utc
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--account-id", help="the AWS account ID of the Panther deployment", required=True
    )
    parser.add_argument(
        "--bucket-name", help="the S3 bucket name of the Panther source", required=True
    )
    parser.add_argument(
        "--region", help="the region of the SQS Queue of the Panther source", required=True
    )
    parser.add_argument(
        "--compromise-datetime",
