...
def rule(event):
    ...
    return description == "Access denied for this IP address"

def title(event):
    user = deep_get(event, "data", "user_name", default="<NO_USER_FOUND>")
    ip = deep_get(event, "data", "ip", default="<NO_ip_FOUND>")
    
    return f'Auth0 {user} attempting to login from restricted country: IP = {ip}'
...
# def alert_context(event):
    ...
    alertcontext = {"description": description, "username": user, "ip address": ip}
    return alertcontext