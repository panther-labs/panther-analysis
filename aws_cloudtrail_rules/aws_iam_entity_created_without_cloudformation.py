...
IAM_ADMIN_ROLE_PATTERNS = { 
    'arn:aws:iam::[0-9]+:role/IdentityCFNServiceRole'
}
...
def rule(event):
    ...
    for admin_role_pattern in IAM_ADMIN_ROLES:
        ...
        if len(re.findall(admin_role_pattern, event['userIdentity']['sessionContext']['sessionIssuer']['arn'])) > 0 : 
            return False

    return event['userIdentity']['sessionContext']['sessionIssuer']['arn'] not in IAM_ADMIN_ROLES
