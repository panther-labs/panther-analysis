...
def rule(event):
    ...
    if event.deep_walk('type') == 'user.login':
        put_string_set(
            ...
            time.time()+DEFAULT_PASSWORD_CHANGE_WINDOW_MINUTES*60)
        return False

    # If we made it here, then this is a password change event
...
def title(event):
    ...
    mins = DEFAULT_PASSWORD_CHANGE_WINDOW_MINUTES
    return f"User [{user_email}] changed their password within [{mins}] minutes of logging in."
