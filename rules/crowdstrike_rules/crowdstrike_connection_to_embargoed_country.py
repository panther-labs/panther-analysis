from panther_base_helpers import deep_get, pattern_match_list, crowdstrike_detection_alert_context
...
def title(event):
    enrichment_obj = get_enrichment_obj(event)
    country_code = [i.get('country') for i in enrichment_obj if i['country'] in EMBARGO_COUNTRY_CODES][0]
    return f"Connection made to embargoed country: {country_code}."

def alert_context(event):
    if event.get('p_log_type') == 'Crowdstrike.FDREvent':
        return crowdstrike_detection_alert_context(event) | {'p_any_ip_addresses': event.get('p_any_ip_addresses')}
    else:
