...
def rule(event):
    # First check if this is a Lambda code update event
    if (
...
def title(event):
    ...
    if identity_type == "IAMUser":
        ...
        return (
            f"[AWS.Lambda.UpdateFunctionCode] IAM User [{user_name}] "
...
def alert_context(event):
    ...
    context["code_source"] = {
        "ecr_image": request_params.get("imageUri"),
        "s3_bucket": request_params.get("s3Bucket"),
        "s3_key": request_params.get("s3Key"),
        "inline_zip": request_params.get("zipFile") is not None,
    }
