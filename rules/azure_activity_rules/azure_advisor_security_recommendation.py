...
def title(event):
    recommendation_name = event.deep_get(
        "properties", "recommendationName", default="<UNKNOWN_RECOMMENDATION>"
    )
    resource_id = event.get("resourceId", "<UNKNOWN_RESOURCE>")

    return f"Azure Advisor Security Recommendation: [{recommendation_name}] for [{resource_id}]"
