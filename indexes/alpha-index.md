...
- [Databricks Session Hijacking - Frequent Logins](../rules/databricks_rules/databricks_session_hijacking_frequent_logins.yml)
  - Detects rapid login events from different public IPs and user agents within a one-hourwindow for the same user. This pattern may indicate session token theft, credentialcompromise, or automated replay of stolen credentials. Uses behavioral baselining tosuppress alerts for users who normally log in from many IPs.Requires: databricks_user_session_baseline_30d lookup table.
...
- [Databricks Spike in Table Admin Activity](../rules/databricks_rules/databricks_spike_in_table_admin_activity.yml)
  - Detects abnormal spikes in admin-level query activity per user against a 30-daybehavioral baseline. Admin queries include DDL/DCL commands (ALTER, DROP, GRANT,REVOKE, CREATE, UPDATE) on tables, schemas, catalogs, and Unity Catalog objects.Requires: databricks_user_query_baseline_30d lookup table.
...
- [Databricks Token Scanning Activity](../rules/databricks_rules/databricks_token_scanning_activity.yml)
  - Detects anomalously low activity from API tokens across multiple public IPs, indicatingbrute-force token probing or scanning. Attackers probe stolen or guessed tokens withsingle requests from many IPs to avoid rate limiting and detection.Requires: databricks_user_session_baseline_30d lookup table.
