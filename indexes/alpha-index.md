...
- [Azure Storage Bulk CPK Blob Upload](../rules/azure_activity_rules/azure_storage_bulk_cpk_blob_upload.yml)
  - Detects bulk blob re-encryption attacks against Azure Storage by tracking the number of distinct blob paths successfully overwritten via PutBlob from the same storage account and caller IP address within a short timeframe. Ransomware actors use Customer-Provided Key (CPK) encryption to overwrite many blobs in rapid succession, holding data hostage while only they possess the decryption key. This rule provides early-warning detection before legitimate users trigger CPK access errors, complementing Azure.Storage.BlobUpload.FOLLOWED-BY.CPKEncryptionError which requires the victim-side error to confirm the attack.
